...
resource "aws_codebuild_project" "deploy_cloudwatch_queries_stage" {
  name           = "${var.project_id}-${var.environment}-deploy-cloudwatch-queries-stage"
  description    = "Deploy CloudWatch Query Definitions"
  build_timeout  = "30"
  queued_timeout = "10"
  service_role   = data.aws_iam_role.pipeline_role.arn
  ...
  environment {
    ...
    # Requires - PROFILE, SHARED_ENVIRONMENT, BLUE_GREEN_ENVIRONMENT and AWS_ACCOUNT to be set
    dynamic "environment_variable" {
      for_each = local.default_environment_variables
      content {
        name  = environment_variable.key
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/aws/codebuild/${var.project_id}-${var.environment}-deploy-cloudwatch-queries-stage"
      stream_name = ""
    }
  }
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "infrastructure/stacks/development-and-deployment-tools/buildspecs/deploy-cloudwatch-queries-buildspec.yml"
  }
}
